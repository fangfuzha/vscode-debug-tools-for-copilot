...
export class AgentConfigurationManager {
  ...
  public async shouldShowPopup(): Promise<boolean> {
    ...
    const popupShown = this.context.globalState.get<boolean>(
      this.POPUP_SHOWN_KEY,
      false,
    );
    return !popupShown;
  }
  ...
  public async showAgentSelectionPopup(): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to show agent selection popup: ${error}`,
      );
    }
  }
  ...
  private getConfigBasePath(): string {
    ...
    switch (platform) {
      ...
      default:
        ...
        console.warn(
          `Unknown platform: ${platform}, using Windows config path`,
        );
        return process.env.APPDATA || path.join(userHome, "AppData", "Roaming");
    }
  }
  ...
  private async getSupportedAgents(): Promise<AgentInfo[]> {
    ...
    console.log(
      `Detected platform: ${platform}, using config base path: ${configBasePath}`,
    );
  ...
  public async migrateExistingConfigurations(): Promise<void> {
    ...
    for (const agent of agents) {
      try {
        ...
        if (needsMigration) {
          console.log(
            `Migrating DebugMCP configuration for ${agent.displayName} from SSE to streamableHttp`,
          );
          ...
          if (
            debugmcpConfig.autoApprove &&
            Array.isArray(debugmcpConfig.autoApprove)
          ) {
          ...
          migrationCount++;
          console.log(
            `Successfully migrated ${agent.displayName} configuration`,
          );
        }
      } catch (error) {
    ...
    try {
      ...
      if (fs.existsSync(agent.configPath)) {
        ...
        try {
          ...
          console.warn(
            `Failed to parse existing config for ${agent.name}, creating new config`,
          );
          config = {};
        }
      }
      ...
      vscode.window.showErrorMessage(
        `Failed to configure DebugMCP for ${agent.displayName}: ${error}`,
      );
      return false;
    }
  }
  ...
  private async configureAgent(agent: AgentInfo): Promise<void> {
    try {
      ...
      vscode.window.showErrorMessage(
        `Failed to configure ${agent.displayName}: ${error}`,
      );
    }
  }
}
